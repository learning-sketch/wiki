...
| ~~sgl-kernel CUDA verify 绑定~~ | ~~sgl-kernel 树已移出本仓库~~ **更正 2026-08-18（re-ingest 补漏）**：sgl-kernel 源树被 `c32c4ef79c`（#32648）**移入 [python/sglang/kernels/aot/](d:\design\sglang\python\sglang\kernels\aot)**（csrc + `sgl_kernel` 绑定齐全，独立打包 `sglang-kernel` wheel、import 名不变）；srt 侧入口 [eagle_utils.py:386](d:\design\sglang\python\sglang\srt\speculative\eagle_utils.py)（CUDA）/ L57（CPU）/ L415（NPU `sgl_kernel_npu`——**这个才是外部包**）/ L341（Triton fallback，kernel 本体在 [kernels/ops/speculative/spec_tree.py](d:\design\sglang\python\sglang\kernels\ops\speculative\spec_tree.py)） |
...
- **`verify_tree_greedy` C++ 算子**：srt 侧入口 [eagle_utils.py:L386（CUDA）/ L415（NPU）](d:\design\sglang\python\sglang\srt\speculative\eagle_utils.py)；~~[sgl-kernel/python/sgl_kernel/speculative.py:38-57] → 顶层 sgl-kernel 路径~~ **更正 2026-08-18**：源树移入仓内 [kernels/aot/python/sgl_kernel/speculative.py](d:\design\sglang\python\sglang\kernels\aot\python\sgl_kernel\speculative.py) → [aot/csrc/common_extension.cc](d:\design\sglang\python\sglang\kernels\aot\csrc\common_extension.cc) → CUDA [aot/csrc/speculative/eagle_utils.cu](d:\design\sglang\python\sglang\kernels\aot\csrc\speculative\eagle_utils.cu)
...
| Verify 机制 | C++ + Plugin | RejectionSampler 三模式 | **`sgl_kernel.verify_tree_greedy` CUDA kernel**（源树在仓内 [kernels/aot/](d:\design\sglang\python\sglang\kernels\aot)，独立 `sglang-kernel` wheel；srt 内另有 Triton fallback） |
