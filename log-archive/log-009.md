# Wiki Activity Log — Archive 009

> 归档自主 `log.md`（2026-08-18 轮转；触发时 997 行 / 114269 字节）。
> 期间：**2026-04-19 全部 8 条 entries**（含 2 条当日 archive 操作记录）。
> 本文件 append-only / 锁死，不允许后续修改。相对链接已按 §11 修复为 `../` 前缀。
...
| 文件 | 期间 | entries | 大小 |
|---|---|---|---|
| **新建** [log-archive/log-005.md](log-005.md) | 2026-04-18（中段补移） | 4 | ~40 KB |
...
**链接修复**：[log-archive/log-005.md](log-005.md) 内 wiki 路径已 rewrite 为相对从 `log-archive/` 出发（`../comparison/...` / `../mindie/...` / `../vllm/...` / `../sglang/...` 等）；`log-archive/log-NNN.md` 路径改为同目录相对 `log-NNN.md`；`d:\design\<...>` 绝对路径不变。
...
| 文件 | 期间 | entries | 大小 |
|---|---|---|---|
| **新建** [log-archive/log-008.md](log-008.md) | 2026-04-19（晚段） | 4 | ~38 KB |
