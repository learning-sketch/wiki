...
- **[topics/speculative.md](sglang/topics/speculative.md)**（~55 锚点，**失效最重**，标 stale）：算法族 5→**7 内建 + 插件**（+DSPARK/FROZEN_KV_MTP）；V1/V2 双轨 → **V1 已删单轨**（统一 `BaseSpecWorker`）；**sgl-kernel 树已移出仓库**。[speculative.md 模块页](sglang/modules/speculative.md) 实测 **48 .py** / enum **8 成员**。新文件 `dflash_disaggregation.py` HEAD 内 0 调用方（VERIFY 已标）。
