...
| vLLM | [d:\design\vllm\](d:\design\vllm) | `main` | `d29dc3ab87840aef42129b30825176295ea73b07` | `d29dc3ab` | 2026-08-18 12:41:26 +0000 | `[Bugfix][Gemma4] Align parser enable_thinking default with template (#52430)` |
...
| vLLM | clean @ `d29dc3ab`（本轮增量从 GitHub `vllm-project/vllm` clone 核对；`5f7fab88..d29dc3ab` 共 4273 commits / wiki 相关子树 ~200 文件 ±6 万行；旧 pin worktree 曾建于 /tmp/vllm-pin 供对比页 cross-check） |
...
cd d:\design\vllm;       git rev-parse HEAD  # 期望 d29dc3ab...
cd d:\design\sglang;     git rev-parse HEAD  # 期望 f7101b0a...
```

如三个 HEAD 都未动 → 已 `verified_against: 2026-08-18` 的 SGLang / vLLM 页仍有效；MindIE 仍对应 2026-04-18 pin。
...
|---|---|---|---|---|---|
| **2026-08-18 (2nd)** | increment | `f032cd3f` (unchanged) | `5f7fab88` → **`d29dc3ab`** | `f7101b0a` (unchanged) | vLLM §12 增量：19 页全处理（13 verified / 4 stale-诚实降级 / spec-decode-eagle+kv-connector 架构重写）；V2 model runner 转正、v0 目录删除、EEP/FT 框架；另 comparison 4 页 SGLang 列刷新 + speculative/parser 重 ingest + 全量 lint（735→245） |
