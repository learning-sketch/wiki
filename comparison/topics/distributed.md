...
| SGLang | 独立模块 [`layers/dp_attention.py`](d:\design\sglang\python\sglang\srt\layers\dp_attention.py)（含 `compute_dp_attention_world_info`、`initialize_dp_attention`、`dp_gather_*` / `dp_scatter` / `dp_reduce_scatter_tensor`）+ `enable_dp_attention` server arg + Scheduler 继承 `SchedulerDPAttnMixin`（[scheduler.py:317-328](d:\design\sglang\python\sglang\srt\managers\scheduler.py)）；mixin `prepare_mlp_sync_batch` 在 TP 组上 all_gather batch 元数据 [scheduler_components/dp_attn.py:228-241](d:\design\sglang\python\sglang\srt\managers\scheduler_components\dp_attn.py) | [layers/dp_attention.py:237-251, 271-302, 311-360, 443-560](d:\design\sglang\python\sglang\srt\layers\dp_attention.py), [data_parallel_controller.py:475-493](d:\design\sglang\python\sglang\srt\managers\data_parallel_controller.py) |
